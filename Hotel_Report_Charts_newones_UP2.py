...
for i in range(0,len(dff['Codice Hotel'])):
    ...
    di3=di3[['Fabbricato',
       ...
       'Terrorismo, Eventi Socio Politici, Atti Dolosi','Franchigia Danni Diretti','Zona rischi catastrofali Nr.', 'Presenza di vincolo', 'Limite di Indennizzo (S.A) Terremoto',
        'Limite di Indennizzo (S.A)  Inondazione, Alluvione, Allagamento']]
    dn3=dN3[dN3.index==i]
    di3=di3.T
    di3=di3.reset_index()
    ...
    di31=di3.loc[[0,1,2,3,4,5,6,8,9,11]]
    #_____________________
    dn31=dn3[dn3.columns[2:9]]
    dc31=dn3[dn3.columns[9:13]]
    x=['Fabbricato','Contenuto']
    y=dn31.loc[i][:2]
    xvals=[0,3]
    ...
    for bari in mybars:
        ...
        if height>.25*HM:
            ...
            plt.gca().text(bari.get_x() + bari.get_width()/2,  1.2*bari.get_height(), locale.format_string('%10.2f', (int(height)), grouping=True), #11000
                         ha='center', color='k', fontsize=10, rotation=90, weight='bold')
        else:
    ...
    for bari in mybars:
        ...
        if height>.25*HM:
            ...
            plt.gca().text(bari.get_x() + bari.get_width()/2, 1.2*bari.get_height(), locale.format_string('%10.2f', (int(height)), grouping=True),
                         ha='center', color='k', fontsize=10, rotation=90, weight='bold')
        else:
    ...
    ax.set_title('Danni diretti (Fabricato, Contenuto, e Catastrofali) [€]')
    #axes = plt.axes()
    #axes.set_xticklabels(['fabbricato','contenuti', 'Ricorso Terzi', 'Cristalli','Furto', 'Merci Refrigerazione'])
    plt.xticks([0.4,3.4], ('Fabbricato','Contenuto'))
    ...
    fname=df['Codice_Unico'][i]+'_plt1_1.png'
    try:
    ...
    table = document.add_table(rows=len(di31)+1, cols=3)
    table.style = 'Light Grid'
    hdr_cells = table.rows[0].cells
    hdr_cells[0].text = di31.columns[0]
    print(2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222)
    #hdr_cells[1].text = di3.columns[1]
    for ti,tii in zip(di31.reset_index().index,di31.index):
        table.cell(ti+1, 0).text=str(di31['SOMME ASSICURATE [€]'][tii])
        table.cell(ti+1, 1).text=str(di31['VALORI ATTUALI'][tii])
    #
    print(2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222)
    table.rows[0].height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
    table.rows[0].height=Inches(0.3)
    for nnn in np.arange(1,len(di31)+1):
        table.rows[nnn].height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
        table.rows[nnn].height=Inches(0.2)
    #--------
    print(2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222)
    ...
    ax.set_xlabel('Partite Assicurate', weight='bold')
    ax.set_ylabel('Valori')
    ax.set_title('Danni diretti (Altri Garanzie) [€]')
    #axes = plt.axes()
    #axes.set_xticklabels(['fabbricato','contenuti', 'Ricorso Terzi', 'Cristalli','Furto', 'Merci Refrigerazione'])
    plt.xticks(xvals, ('Ricorso\n Terzi', 'Cristalli','Furto', 'Merci\n Refrigerazione', 'Fenomeno\n Elettrico'))
    for spine in plt.gca().spines.values():
        spine.set_visible(False)
    #
    plt.gca().axes.yaxis.set_visible(False)
    plt.gca().tick_params(axis='both', which='both', length=0)
    #matplotlib.style.use('default') ######################################################################################################################
    fname=df['Codice_Unico'][i]+'_plt1_2.png'
    try:
        fig.savefig(bbox_inches='tight',fname=fname)
    except:
        fig.savefig(fname=fname)
    ...
    print(2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222)
    plt.close('all')
    di32=di3.loc[[7,10,12,13]]
    for mll in [12,13]:
        di32['VALORI ATTUALI'].loc[mll]="{:.0%}".format(di32['VALORI ATTUALI'].loc[mll])
    #di32['VALORI ATTUALI'].loc[[12,13]]=(di32.loc[[12,13]]['VALORI ATTUALI']*100).astype(int)
    document.add_paragraph('Danni Catastrofali')
    table = document.add_table(rows=len(di32)+1, cols=3)
    table.style = 'Light Grid'
    hdr_cells = table.rows[0].cells
    hdr_cells[0].text = di32.columns[0]
    #hdr_cells[1].text = di3.columns[1]
    for ti,tii in zip(di32.reset_index().index,di32.index):
        table.cell(ti+1, 0).text=str(di32['SOMME ASSICURATE [€]'][tii])
        table.cell(ti+1, 1).text=str(di32['VALORI ATTUALI'][tii])
    #
    table.rows[0].height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
    table.rows[0].height=Inches(0.3)
    for nnn in np.arange(1,len(di32)+1):
        table.rows[nnn].height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
        table.rows[nnn].height=Inches(0.2)
    #--------
    document.add_paragraph('')
    ...
    plt.figure(1)
    #plt.yscale('log')
    mybars=plt.bar(xvals,y,width=0.2,alpha=0.8) #fuchsia, ##18BA2B, #brown
    x=plt.gca().xaxis
    Hh=[]
    for bari in mybars:
        Hh.append(bari.get_height())
