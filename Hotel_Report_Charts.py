...
df=pd.read_excel(r"C:\Users\DataBase_2_Final.xlsx")
df=df[df.columns[1:]]
...
'Franchigia (RCT)', 'Premio lordo Annuo 2019/2020']
...
##
##
Dati=['Fatturato  Hotel 2019 (€)', 'Fatturato Ristorante 2019 (€)','Franchigia (RCT)']
...
for i in range(0,len(dff['Codice Hotel'])):
    ...
    for bari in mybars:
        ...
        if height>40000 and height>0.2*HM:
            plt.gca().text(bari.get_x() + bari.get_width()/2,  0.5*bari.get_height(), locale.format_string('%10.2f', (int(height)), grouping=True),
                         ha='center', color='w', fontsize=10, rotation=90, weight='bold')
        else:
    ...
    plt.xticks(xvals, ('Hotel', 'Ristorante'))
